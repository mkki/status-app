...
describe('uploadPhoto', () => {
  ...
  it('authToken 전달 시 Authorization 헤더에 Bearer로 포함', async () => {
    ...
    fetchMock.mockResolvedValue({
      ok: true,
      json: async () => ({ url: 'https://cdn.example.com/p.jpg' }),
    });
  ...
  it('FormData 본문에 questId와 file 필드가 포함됨', async () => {
    ...
    expect(body.get('questId')).toBe('q1');
  ...
  it('응답에 url 필드가 없으면 throw', async () => {
    const { uploadPhoto } = loadUploader(TEST_ENDPOINT);
...
describe('uploadPhotos', () => {
  ...
  it('성공/실패가 섞인 결과를 입력 순서대로 반환', async () => {
    ...
    fetchMock
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({ url: 'https://cdn/a.jpg' }),
      })
      .mockResolvedValueOnce({
        ok: false,
        status: 500,
      ...
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({ url: 'https://cdn/c.jpg' }),
      });
  ...
  it('업로드는 순차적으로 진행됨 (병렬 아님)', async () => {
    ...
    fetchMock.mockImplementation(async (_url, init: RequestInit) => {
      const body = init.body as FormData;
      callOrder.push(body.get('questId') as string);
      await new Promise((r) => setTimeout(r, 10));
      return { ok: true, json: async () => ({ url: 'https://cdn/x.jpg' }) };
    });
    ...
    expect(callOrder).toEqual(['q1', 'q1']);
  });
